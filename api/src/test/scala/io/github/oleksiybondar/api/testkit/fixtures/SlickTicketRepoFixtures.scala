...
import io.github.oleksiybondar.api.infrastructure.db.ticket.SlickTicketRepo
import slick.jdbc.PostgresProfile.api._

import scala.concurrent.ExecutionContext

object SlickTicketRepoFixtures {

  given ExecutionContext = ExecutionContext.global

  private val repoResource: Resource[IO, (Database, SlickTicketRepo[IO])] =
    TestDatabaseSupport.databaseResource.map(db => (db, new SlickTicketRepo[IO](db)))

  def withCleanRepo[A](run: SlickTicketRepo[IO] => IO[A]): A =
    repoResource
      .use { case (db, repo) =>
