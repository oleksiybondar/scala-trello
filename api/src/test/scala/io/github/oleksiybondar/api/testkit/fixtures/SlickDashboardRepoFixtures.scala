...
import io.github.oleksiybondar.api.infrastructure.db.dashboard.SlickDashboardRepo
import org.flywaydb.core.Flyway
import slick.jdbc.PostgresProfile.api._

import scala.concurrent.ExecutionContext

object SlickDashboardRepoFixtures {
  ...
  private val repoResource: Resource[IO, (Database, SlickDashboardRepo[IO])] =
    databaseResource.map { db =>
      (db, new SlickDashboardRepo[IO](db))
    }

  def withCleanRepo[A](run: SlickDashboardRepo[IO] => IO[A]): A =
    repoResource
      .use { case (db, repo) =>
