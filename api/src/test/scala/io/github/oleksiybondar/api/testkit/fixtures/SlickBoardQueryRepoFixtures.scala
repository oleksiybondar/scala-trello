...
import io.github.oleksiybondar.api.infrastructure.db.board.SlickBoardQueryRepo
import slick.jdbc.PostgresProfile.api._

import scala.concurrent.ExecutionContext

object SlickBoardQueryRepoFixtures {

  given ExecutionContext = ExecutionContext.global

  private val repoResource: Resource[IO, (Database, SlickBoardQueryRepo[IO])] =
    TestDatabaseSupport.databaseResource.map(db => (db, new SlickBoardQueryRepo[IO](db)))

  def withCleanRepo[A](run: SlickBoardQueryRepo[IO] => IO[A]): A =
    repoResource
      .use { case (db, repo) =>
        TestDatabaseSupport.clearDatabase(db) *> seedBaseData(db) *> run(
          repo
        ).guarantee(TestDatabaseSupport.clearDatabase(db))
      }
      .unsafeRunSync()

  def withCleanRepoAndDatabase[A](run: (Database, SlickBoardQueryRepo[IO]) => IO[A]): A =
    repoResource
      .use { case (db, repo) =>
        TestDatabaseSupport.clearDatabase(db) *> seedBaseData(db) *> run(
          db,
          ...
          DBIO.seq(
            sqlu"""
              ...
              VALUES
                ...
                ('22222222-2222-2222-2222-222222222222', 'bob', 'bob@example.com', 'hash:secret', 'Bob', 'Builder', 'https://example.com/bob.png', TIMESTAMPTZ '2026-04-05T08:30:00Z')
            """,
            sqlu"""
            ...
            sqlu"""
              ...
              VALUES
                (1, 'admin', 'Full board access'),
                (2, 'contributor', 'Standard team member access')
            """,
            sqlu"""
              INSERT INTO permissions (id, role_id, area, can_read, can_create, can_modify, can_delete, can_reassign)
              VALUES
                (1, 1, 'dashboard', TRUE, TRUE, TRUE, TRUE, FALSE),
            ...
            sqlu"""
              ...
              VALUES
                ('aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa', '11111111-1111-1111-1111-111111111111', 1, TIMESTAMPTZ '2026-04-06T08:00:00Z'),
                ('aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa', '22222222-2222-2222-2222-222222222222', 2, TIMESTAMPTZ '2026-04-06T08:05:00Z')
            """,
            sqlu"""
