...
import io.github.oleksiybondar.api.infrastructure.db.dashboard.SlickDashboardMemberRepo
import org.flywaydb.core.Flyway
import slick.jdbc.PostgresProfile.api._

import scala.concurrent.ExecutionContext

object SlickDashboardMemberRepoFixtures {
  ...
  private val repoResource: Resource[IO, (Database, SlickDashboardMemberRepo[IO])] =
    databaseResource.map(db => (db, new SlickDashboardMemberRepo[IO](db)))

  def withCleanRepo[A](run: SlickDashboardMemberRepo[IO] => IO[A]): A =
    repoResource
      .use { case (db, repo) =>
  ...
  private def clearDatabase(db: Database): IO[Unit] =
    IO.fromFuture(
      IO(
        db.run(
          sqlu"TRUNCATE TABLE time_tracking, comments, tickets, dashboard_members, dashboards, permissions, roles, activities, severities, states, auth_sessions, password_history, users CASCADE"
        )
      )
    ).void
      ...
      IO(
        db.run(
          DBIO.seq(
            ...
            sqlu"""
              INSERT INTO dashboards (id, name, description, active, owner_user_id, created_by_user_id, created_at, modified_at, last_modified_by_user_id)
              VALUES
