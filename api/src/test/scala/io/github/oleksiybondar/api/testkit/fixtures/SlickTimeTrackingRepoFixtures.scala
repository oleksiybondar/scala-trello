...
import io.github.oleksiybondar.api.infrastructure.db.timeTracking.SlickTimeTrackingRepo
import slick.jdbc.PostgresProfile.api._

import scala.concurrent.ExecutionContext

object SlickTimeTrackingRepoFixtures {

  given ExecutionContext = ExecutionContext.global

  private val repoResource: Resource[IO, (Database, SlickTimeTrackingRepo[IO])] =
    TestDatabaseSupport.databaseResource.map(db => (db, new SlickTimeTrackingRepo[IO](db)))

  def withCleanRepo[A](run: SlickTimeTrackingRepo[IO] => IO[A]): A =
    repoResource
      .use { case (db, repo) =>
