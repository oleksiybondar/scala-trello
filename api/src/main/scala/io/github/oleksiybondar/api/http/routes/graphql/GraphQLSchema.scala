...
object GraphQLSchema {
  ...
  private val QueryType: ObjectType[GraphQLContext, Unit] =
    ObjectType(
      ...
      fields = fields[GraphQLContext, Unit](
        (UserApi.queryFields ++ RoleApi.queryFields ++ BoardApi.queryFields ++ TicketApi.queryFields)*
      )
    )
  ...
  private val MutationType: ObjectType[GraphQLContext, Unit] =
    ObjectType(
      name = "Mutations",
      fields = fields[GraphQLContext, Unit](
        (UserApi.mutationFields ++ BoardApi.mutationFields ++ TicketApi.mutationFields)*
      )
    )
