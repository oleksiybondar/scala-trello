...
import io.github.oleksiybondar.api.infrastructure.db.comment.SlickCommentRepo
import slick.jdbc.PostgresProfile.api._

import scala.concurrent.ExecutionContext

object SlickCommentRepoFixtures {

  given ExecutionContext = ExecutionContext.global

  private val repoResource: Resource[IO, (Database, SlickCommentRepo[IO])] =
    TestDatabaseSupport.databaseResource.map(db => (db, new SlickCommentRepo[IO](db)))

  def withCleanRepo[A](run: SlickCommentRepo[IO] => IO[A]): A =
    repoResource
      .use { case (db, repo) =>
